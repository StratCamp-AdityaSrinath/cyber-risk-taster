...
try:
    ...
    df_cyber = df_long.pivot_table(
        index=['Year', 'Industry NAICS', 'Code', 'Remedial Service Type'],
        columns='Metric',
        values='Value'
    ).reset_index()
    ...
    # --- NEW OPTIMIZATION ---
    # Keep only the columns the API actually needs to reduce file size
    final_columns = [
        'Year', 'Industry NAICS', 'Code', 'Remedial Service Type', 
        'Event_Freq', 'Cost_mu', 'Cost_sigma'
    ]
